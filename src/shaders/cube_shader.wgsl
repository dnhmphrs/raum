...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let color1 = mix(0.5 + 0.5 * pattern1, pattern2, sin(time.time * clip_pos_truncated.x * clip_pos_truncated.y * 0.01));
    let color2 = mix(pattern2, pattern3, cos(time.time * clip_pos_truncated.y * 0.01));
