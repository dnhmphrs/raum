...
fn vs_main(
    ...
    let displaced_position = vec3<f32>(model.position.x, model.position.y + displacement, model.position.z);
...
fn fs_main(in: VertexOutput) -> @location(0) vec4<f32> {
    ...
    let final_color = vec4<f32>(1.0 - color1, 1.0 - color2 * height, 1.0 - color3, 1.0);
